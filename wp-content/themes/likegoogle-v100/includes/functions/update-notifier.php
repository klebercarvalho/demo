...
function update_notifier_menu() {  
	if ( function_exists('simplexml_load_string') && function_exists('wp_get_theme') ) { // Stop if simplexml_load_string funtion isn't available
	    $xml = get_latest_theme_version(SILICONTHEMES_NOTIFIER_CACHE_INTERVAL); // Get the latest remote XML file on our server
...
function update_notifier_bar_menu() {
	if (function_exists('simplexml_load_string') && function_exists('wp_get_theme') ) { // Stop if simplexml_load_string funtion isn't available
		...
		if( version_compare($xml->latest, $theme_data['Version'], '>') ) { // Compare current theme version with the remote XML version
			$wp_admin_bar->add_menu( array( 'id' => 'update_notifier', 'title' => '<span>' . SILICONTHEMES_NOTIFIER_THEME_NAME . ' <span id="ab-updates">1 Update</span></span>', 'href' => get_admin_url() . 'admin.php?page=theme-update-notifier' ) );
		}
	}
}
...
function update_notifier() { 
	...
	<div class="wrap">
		...
		<h2><?php echo SILICONTHEMES_NOTIFIER_THEME_NAME ?> Theme Updates</h2>
	    <div id="message" class="updated below-h2"><p><strong>There is a new version of the <?php echo SILICONTHEMES_NOTIFIER_THEME_NAME; ?> theme available.</strong> You have version <?php echo $theme_data['Version']; ?> installed. Update to version <?php echo $xml->latest; ?>.</p></div>
		...
		<div id="instructions">
		    ...
		    <p><strong>Please note:</strong> make a <strong>backup</strong> of the Theme inside your WordPress installation folder <code>/wp-content/themes/<?php echo SILICONTHEMES_NOTIFIER_THEME_FOLDER_NAME; ?>/</code></p>
			...
			<p><h3>UPDATE</h3> Upload the new files using FTP to the <code>/wp-content/themes/<?php echo SILICONTHEMES_NOTIFIER_THEME_FOLDER_NAME; ?>/</code> folder overwriting the old ones .</p>
...
function get_latest_theme_version($interval) {
	$notifier_file_url = SILICONTHEMES_NOTIFIER_XML_FILE;	
