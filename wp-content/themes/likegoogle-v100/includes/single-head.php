...
if( $get_meta['sith_post_head'][0] != 'none' ):
	
	$orig_post = $post;
	
	//Get Post Video
	if( $get_meta['sith_post_head'][0] == 'video' ){ ?>
		<div class="single-post-video">
			<?php sith_video(); ?>
		</div>
	<?php	
	}elseif( $get_meta['sith_post_head'][0] == 'audio' ){
		if( !empty( $get_meta["sith_audio_mp3"][0] ) || !empty( $get_meta["sith_audio_m4a"][0] ) || !empty( $get_meta["sith_audio_oga"][0] ) ){
			...
			<div class="single-post-audio single-post-thumb">
				<?php sith_thumb( $size ); ?>
				<div class="single-audio"<?php echo $style ?>><?php sith_audio(); ?></div>
			</div>
	<?php
		}
	}elseif( $get_meta['sith_post_head'][0] == 'soundcloud' ){
		if( !empty( $get_meta["sith_audio_soundcloud"][0] ) ){
			$play = 'false';
			if( !empty( $get_meta["sith_audio_soundcloud_play"][0] )) $play = 'true';
			echo sith_soundcloud($get_meta["sith_audio_soundcloud"][0] , $play );?>
	<?php
		}
	}elseif( $get_meta['sith_post_head'][0] == 'thumb' || ( empty( $get_meta['sith_post_head'][0] ) && sith_get_option( 'post_featured' ) ) ){
		if( $get_meta["sith_sidebar_pos"][0] == 'full' ){
			$size = 'big-slider';
		}else{
			$size = 'slider';
...
<?php }elseif( $get_meta['sith_post_head'][0] == 'lightbox' && has_post_thumbnail($post->ID)){
		...
		<div class="single-post-thumb head-lightbox">
			<a href="<?php echo $image_url; ?>" rel="prettyPhoto"><?php sith_thumb( $size ); ?></a>
		</div>
		<?php $thumb_caption = get_post(get_post_thumbnail_id())->post_excerpt;
			if( !empty($thumb_caption) ){ ?><div class="single-post-caption"><?php echo $thumb_caption ?></div> <?php } ?>
			
<?php } elseif( $get_meta['sith_post_head'][0] == 'map' && !empty( $get_meta['sith_googlemap_url'][0] ) ){
		if( $get_meta["sith_sidebar_pos"][0] == 'full' ){
			$width = 1003 ;
			$height = 498 ;
		}else{
		...
		<?php echo sith_google_maps( $get_meta['sith_googlemap_url'][0] , $width , $height ); ?>
		
		
<?php }elseif( $get_meta['sith_post_head'][0] == 'slider' && !empty( $get_meta['sith_post_slider'][0] ) ){
	...
	<div class="flexslider" id="flexslider-post">
		...
		<?php while ( $custom_slider->have_posts() ) : $custom_slider->the_post();
			...
			<li>
				...
				<img src="<?php echo sith_slider_img_src( $slide['id'] , $size ) ?>" alt="" />
