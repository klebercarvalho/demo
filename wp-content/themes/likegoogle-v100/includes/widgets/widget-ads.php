...
class ads125_widget extends WP_Widget {
	function ads125_widget() {
		...
		$this->WP_Widget( 'ads125-widget',/*theme_name .*/' [st] Ads 125*125', $widget_ops, $control_ops );
	}
...
<div <?php if( $tran_bg ) echo 'id="'.$args['widget_id'].'"'; ?> class="ads125<?php echo $one_column ?>">
  ...
  <div class="ad-cell"> <?php echo do_shortcode( $instance[ 'ads'.$i.'_code' ] ); ?> </div>
  <?php } elseif( !empty($instance[ 'ads'.$i.'_img' ])  ){ ?>
  <div class="ad-cell">
    <?php if( $instance[ 'ads'.$i.'_url' ] ){ ?>
    <a href="<?php echo $instance[ 'ads'.$i.'_url' ] ?>" <?php echo $new_window ?> <?php echo $nofollow ?>>
    <?php } ?>
...
<?php 
	...
	function form( $instance ) {
		$defaults = array( 'title' => __(' Advertisement', 'sith') );
...
<p>
  ...
  <br />
  <small>if this active the title will disappear</small> </p>
<p>
...
<p>
  ...
  <textarea id="<?php echo $this->get_field_id( 'ads'.$i.'_code' ); ?>" name="<?php echo $this->get_field_name( 'ads'.$i.'_code' ); ?>" class="widefat" ><?php if( !empty( $instance['ads'.$i.'_code'] ) ) echo $instance['ads'.$i.'_code']; ?>
</textarea>
</p>
<?php } ?>
<?php
...
class ads300_600_widget extends WP_Widget {
	function ads300_600_widget() {
		...
		$this->WP_Widget( 'ads300_600-widget',/*theme_name .*/' [st] Ads 300*600', $widget_ops, $control_ops );
	}
...
class ads300_100_widget extends WP_Widget {
	function ads300_100_widget() {
		...
		$this->WP_Widget( 'ads300_100-widget',/*theme_name .*/' [st] Ads 300*100', $widget_ops, $control_ops );
	}
...
class ads300_250_widget extends WP_Widget {
	function ads300_250_widget() {
		...
		$this->WP_Widget( 'ads300_250-widget',/*theme_name .*/' [st] Ads 300*250', $widget_ops, $control_ops );
	}
