<?php
add_action( 'widgets_init', 'sith_categort_posts_widget' );
function sith_categort_posts_widget() {
	register_widget( 'sith_categort_posts' );
}
class sith_categort_posts extends WP_Widget {

	function sith_categort_posts() {
		$widget_ops = array( 'classname' => 'category-posts' );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'category-posts-widget' );
		$this->WP_Widget( 'category-posts-widget',/*theme_name .*/' [st] Category Posts', $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<?php echo $after_title; ?>
				<ul>
					<?php sith_last_posts_cat($no_of_posts , $thumb , $cats_id)?>	
				</ul>
		<div class="clear"></div>
	<?php 
		echo $after_widget;
	...
	function form( $instance ) {
		$defaults = array( 'title' =>__( 'Category Posts' , 'sith'), 'no_of_posts' => '5' , 'cats_id' => '1' , 'thumb' => 'true' );
