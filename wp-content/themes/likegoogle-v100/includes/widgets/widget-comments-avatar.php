<?php
add_action( 'widgets_init', 'sith_comments_avatar_widget' );
function sith_comments_avatar_widget() {
	register_widget( 'sith_comments_avatar' );
}
class sith_comments_avatar extends WP_Widget {

	function sith_comments_avatar() {
		$widget_ops = array( 'classname' => 'comments-avatar' );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'comments_avatar-widget' );
		$this->WP_Widget( 'comments_avatar-widget',/*theme_name .*/' [st] Recent Comments with avatar', $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<?php echo $after_title; ?>
			<ul>	
		<?php sith_most_commented( $no_of_comments , $avatar_size); ?>
		</ul>
	<?php 
		echo $after_widget;
	}
	...
	function form( $instance ) {
		$defaults = array( 'title' =>__( 'Recent Comments' , 'sith'), 'no_of_comments' => '5' , 'avatar_size' => '50' );
