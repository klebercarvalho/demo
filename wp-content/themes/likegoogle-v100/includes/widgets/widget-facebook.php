...
add_action( 'widgets_init', 'sith_acebook_widget_box' );
function sith_acebook_widget_box() {
	register_widget( 'sith_facebook_widget' );
}
class sith_facebook_widget extends WP_Widget {

	function sith_facebook_widget() {
		$widget_ops = array( 'classname' => 'facebook-widget' );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'facebook-widget' );
		$this->WP_Widget( 'facebook-widget',/*theme_name .*/' [st] Facebook', $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		$defaults = array( 'title' =>__( 'Find us on Facebook' , 'sith') );
