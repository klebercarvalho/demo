...
add_action( 'widgets_init', 'sith_google_widget_box' );
function sith_google_widget_box() {
	register_widget( 'sith_google_widget' );
}
class sith_google_widget extends WP_Widget {

	function sith_google_widget() {
		$widget_ops = array( 'classname' => 'google-widget'  );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'google-widget' );
		$this->WP_Widget( 'google-widget',/*theme_name .*/' [st] Google + page', $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		$defaults = array( 'title' =>__( 'Follow us on Google+' , 'sith') );
