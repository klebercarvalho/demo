<?php
add_action( 'widgets_init', 'sith_posts_list_widget' );
function sith_posts_list_widget() {
	register_widget( 'sith_posts_list' );
}
class sith_posts_list extends WP_Widget {

	function sith_posts_list() {
		$widget_ops = array( 'classname' => 'posts-list'  );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'posts-list-widget' );
		$this->WP_Widget( 'posts-list-widget',/*theme_name .*/' [st] Posts list', $widget_ops, $control_ops );
	}
		...
		<?php echo $after_title; ?>
				<ul>
					...
					if( $posts_order == 'popular' )
						sith_popular_posts($no_of_posts , $thumb);
						
					elseif( $posts_order == 'random' )
						sith_random_posts($no_of_posts , $thumb);
						
					elseif( $posts_order == 'best' )
						sith_best_reviews_posts($no_of_posts , $thumb);
						
					else
						sith_last_posts($no_of_posts , $thumb)?>	
				</ul>
		<div class="clear"></div>
	<?php 
		echo $after_widget;
	...
	function form( $instance ) {
		$defaults = array( 'title' =>__('Recent Posts' , 'sith') , 'no_of_posts' => '5' , 'posts_order' => 'latest', 'thumb' => 'true' );
