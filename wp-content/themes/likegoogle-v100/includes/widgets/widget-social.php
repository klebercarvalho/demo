<?php
add_action( 'widgets_init', 'sith_social_widget_box' );
function sith_social_widget_box() {
	register_widget( 'sith_social_widget' );
}
class sith_social_widget extends WP_Widget {

	function sith_social_widget() {
		$widget_ops = array( 'classname' => 'social-icons-widget' );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'social' );
		$this->WP_Widget( 'social',/*theme_name .*/' [st] Social Icons', $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if( !$tran_bg ){
			...
			echo $title ; 
			echo $after_title;
				sith_get_social($newtab= $newtap, $icon_size=$icons_size);
			echo $after_widget;
		}
		else { ?>
			<div class="widget social-icons-widget">
			<?php sith_get_social($newtab= $newtap, $icon_size=$icons_size); ?>
			</div>
		<?php }			
	}
	...
	function form( $instance ) {
		$defaults = array( 'title' =>__('Social' , 'sith') , 'icon_size' =>'16' );
