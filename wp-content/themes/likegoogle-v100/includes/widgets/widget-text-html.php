<?php
add_action( 'widgets_init', 'sith_text_html_widget' );
function sith_text_html_widget() {
	register_widget( 'sith_text_html' );
}
class sith_text_html extends WP_Widget {

	function sith_text_html() {
		$widget_ops = array( 'classname' => 'text-html'  );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'text-html-widget' );
		$this->WP_Widget( 'text-html-widget',/*theme_name .*/' [st] Text or Html', $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		$defaults = array( 'title' =>__('Text' , 'sith')  );
