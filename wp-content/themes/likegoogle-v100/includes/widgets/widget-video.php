...
add_action( 'widgets_init', 'sith_video_widget_box' );
function sith_video_widget_box() {
	register_widget( 'sith_video_widget' );
}
class sith_video_widget extends WP_Widget {

	function sith_video_widget() {
		$widget_ops = array( 'classname' => 'video-widget', 'description' => ''  );
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'video-widget' );
		$this->WP_Widget( 'video-widget',/*theme_name .*/' [st] Video', $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		$defaults = array( 'title' =>__(' Featured Video', 'sith') );
