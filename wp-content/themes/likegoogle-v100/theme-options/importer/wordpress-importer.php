	...
	function process_posts() {
		foreach ( $this->posts as $post ) {
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				echo '<br />';
				$lang_comment_post_ID = $post_id = $post_exists;
			} else {
				...
				if ( 'attachment' == $postdata['post_type'] ) {
					...
					$lang_comment_post_ID = $post_id = $this->process_attachment( $postdata, $remote_url );
				} else {
					$lang_comment_post_ID = $post_id = wp_insert_post( $postdata, true );
				}
			...
			if ( ! empty( $post['comments'] ) ) {
				...
				foreach ( $post['comments'] as $lang_comment ) {
					$lang_comment_id	= $lang_comment['comment_id'];
					$newcomments[$lang_comment_id]['comment_post_ID']      = $lang_comment_post_ID;
					$newcomments[$lang_comment_id]['comment_author']       = $lang_comment['comment_author'];
					$newcomments[$lang_comment_id]['comment_author_email'] = $lang_comment['comment_author_email'];
					$newcomments[$lang_comment_id]['comment_author_IP']    = $lang_comment['comment_author_IP'];
					$newcomments[$lang_comment_id]['comment_author_url']   = $lang_comment['comment_author_url'];
					$newcomments[$lang_comment_id]['comment_date']         = $lang_comment['comment_date'];
					$newcomments[$lang_comment_id]['comment_date_gmt']     = $lang_comment['comment_date_gmt'];
					$newcomments[$lang_comment_id]['comment_content']      = $lang_comment['comment_content'];
					$newcomments[$lang_comment_id]['comment_approved']     = $lang_comment['comment_approved'];
					$newcomments[$lang_comment_id]['comment_type']         = $lang_comment['comment_type'];
					$newcomments[$lang_comment_id]['comment_parent'] 	  = $lang_comment['comment_parent'];
					$newcomments[$lang_comment_id]['commentmeta']          = isset( $lang_comment['commentmeta'] ) ? $lang_comment['commentmeta'] : array();
					if ( isset( $this->processed_authors[$lang_comment['comment_user_id']] ) )
						$newcomments[$lang_comment_id]['user_id'] = $this->processed_authors[$lang_comment['comment_user_id']];
				}
				ksort( $newcomments );

				foreach ( $newcomments as $key => $lang_comment ) {
					// if this is a new post we can skip the comment_exists() check
					if ( ! $post_exists || ! comment_exists( $lang_comment['comment_author'], $lang_comment['comment_date'] ) ) {
						if ( isset( $inserted_comments[$lang_comment['comment_parent']] ) )
							$lang_comment['comment_parent'] = $inserted_comments[$lang_comment['comment_parent']];
						$lang_comment = wp_filter_comment( $lang_comment );
						$inserted_comments[$key] = wp_insert_comment( $lang_comment );

						foreach( $lang_comment['commentmeta'] as $meta ) {
